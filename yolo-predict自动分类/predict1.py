        ...
        for img_name in tqdm(img_names):
            if img_name.lower().endswith(('.bmp', '.dib', '.png', '.jpg', '.jpeg', '.pbm', '.pgm', '.ppm', '.tif', '.tiff')):
                ...
                if any(flag):  # any()语法： 只要flag中存在True时即返回True
                    r_image.save(os.path.join(dir_save_path_conf, img_name.replace(".jpg", ".png")), quality=95, subsampling=0)
                    success += 1
                else:
                    r_image.save(os.path.join(dir_save_path_fail_conf, img_name.replace(".jpg", ".png")), quality=95,subsampling=0)
                    fail += 1
